...
The generated room graph is not a loading-screen trick. It is the serialized level contract used by gameplay, rendering, replay, export, testing, and eventually the backend. Every run now combines two independently generated states that can physically overlap after the safe window closes.

## What is playable now

### Instituto Travessia headquarters

The prototype now starts and returns at a persistent headquarters interface.

- Six route authorizations across the Lusophone setting, each with distinct room count, graph density, safe-window timing, risk, and route payout.
- Persistent field rank, experience, scrip, run totals, remote-object totals, and audit statistics.
...
- Permanent permissions that alter cooldowns, healing, interlace timing, remote-object value, seizure protection, remote-vault rewards, and The Auditor.
...
- Generated field contracts with local-room, remote-room, overlap, vault, remote-vault, object, value, and Auditor requirements.
- A safe window followed by an independently generated server interlace.
- Extraction instability that allows A Chave Geral to seize contradictory objects when pressure remains unresolved.
